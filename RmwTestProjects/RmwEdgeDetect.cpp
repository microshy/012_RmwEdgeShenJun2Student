...
{	BYTE *pCur1,*pCur2;
	...
	//Color = 255;
	for (i = 1; i < a; i++) {
		for (x = i-1, pCur1 = BinImg+(i - 1) * width, pCur2 = BinImg + (height - i)*width; x < width; x++) {
			//pCur2 = BinImg + (height - (long)i)*width;
			*(pCur1++) = *(pCur2++) = Color;
		} 
	}
	for (j = 1; j < a; j++) {
		for (y = j-1, pCur1 = BinImg+j-1, pCur2 = BinImg + width - j; y < height; y++, pCur1 += width, pCur2 += width) {
			*pCur1 = *pCur2 = Color;
		} 
	}
...
{ 
   ...
   x2=width-1;
   y2=height-1;
   XsySetImageBoundary(pRbImg,width,height,0);
...
{ 
   ...
   x2=width-1;
   y2=height-1;
   XsySetImageBoundary(pSbImg,width,height,0);
...
//生成一个LUTA的加法表和一个LUT的min表可以减少运算
//利用边界函数是不是就可以省去首行首列尾行尾列的工作
